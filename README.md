...
Note that schemata's [`schemata-codegen`](https://github.com/nostrability/schemata-codegen) pipeline can generate native validators from JSON-Schema, combining the correctness of a standards-based specification with the performance of compiled code. For performance-sensitive use cases, [`schemata-codegen`](https://github.com/nostrability/schemata-codegen) can generate native structural validators in 13 languages that don't use JSON-Schema at runtime.
